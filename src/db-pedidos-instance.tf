resource "aws_db_instance" "snacktech_db_pedidos" {
  ...
  identifier                = "${var.projectName}-db-pedidos"
  username                  = var.dbUserPedidos
  password                  = sensitive(var.dbPasswordPedidos)
...
output "db_instance_pedidos" {
  value = aws_db_instance.snacktech_db_pedidos.address
  sensitive = true
}

output "db_port_pedidos" {
  value = aws_db_instance.snacktech_db_pedidos.port
}